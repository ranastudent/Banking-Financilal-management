...
const createTestUser = async (
  ...
  const email = `deposit-idempotency-${Date.now()}-${Math.random()}@test.local`;
  ...
  const user = await prisma.user.create({
    data: {
      name: `Deposit Idempotency User ${Date.now()}-${Math.random()}`,
      email,
      passwordHash,
      role,
...
const createTestAccount = async (
  ...
  const account = await prisma.account.create({
    data: {
      userId,
      accountNumber: `DIDEM-${Date.now()}-${Math.floor(
        Math.random() * 100000,
      )}`,
      accountType: "SAVINGS",
      status: "ACTIVE",
...
describe("12.9 Deposit Idempotency", () => {
  it("should reject a deposit request without Idempotency-Key", async () => {
  ...
  it("should create an idempotency record on the first successful deposit", async () => {
    const user = await createTestUser();
    ...
    const idempotencyKey = `deposit-idem-first-${Date.now()}-${Math.random()}`;
    ...
    expect(idempotencyRecord.userId).toBe(user.id);
    expect(idempotencyRecord.requestHash).toMatch(
      /^[a-f0-9]{64}$/,
    );
    ...
    expect(idempotencyRecord.expiresAt.getTime()).toBeGreaterThan(
      Date.now(),
    );
  });

  it("should return the previous response and not process the deposit twice", async () => {
    const user = await createTestUser();
    ...
    const idempotencyKey = `deposit-idem-replay-${Date.now()}-${Math.random()}`;
    ...
    ).toBe(firstResponse.body.data.amount);

    expect(
      secondResponse.body.data.accountId,
    ).toBe(firstResponse.body.data.accountId);
  ...
  it("should reject reuse of the same key with a different request", async () => {
    ...
    const idempotencyKey = `deposit-idem-conflict-${Date.now()}-${Math.random()}`;
    ...
    createdTransactionIds.push(firstTransactionId);
  ...
  it("should allow the same Idempotency-Key for different users", async () => {
    ...
    const tokenA = getAccessToken(userA);
    const tokenB = getAccessToken(userB);

    const idempotencyKey = `deposit-idem-user-scope-${Date.now()}-${Math.random()}`;

    const responseA = await request(app)
      .post(`/api/v1/accounts/${accountA.id}/deposits`)
      .set(
        "Authorization",
        `Bearer ${tokenA}`,
      )
    ...
    createdTransactionIds.push(transactionA);
    ...
    createdTransactionIds.push(transactionB);

    expect(transactionB).not.toBe(
      transactionA,
    );
    ...
    expect(records[0]?.userId).toBe(userA.id);
    expect(records[1]?.userId).toBe(userB.id);
  });
  ...
  it("should store the original response body and status", async () => {
    ...
    const idempotencyKey = `deposit-idem-response-${Date.now()}-${Math.random()}`;
    ...
    createdIdempotencyRecordIds.push(record.id);
    ...
    expect(responseBody.accountId).toBe(
      account.id,
    );

    expect(responseBody.amount).toBe("250");

    expect(responseBody.currency).toBe("BDT");

    expect(responseBody.transaction?.id).toBe(
      transactionId,
    );
  });
});