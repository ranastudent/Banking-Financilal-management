...
afterEach(async () => {
  /*
   * ------------------------------------------------------------------
   * 1. Discover ALL FX requests belonging to our test accounts.
   *
   * This makes cleanup resilient even if a test partially failed after
   * creating the request but before adding its ID to the tracking array.
   * ------------------------------------------------------------------
   */
  let requestIdsToDelete =
    [...createdRequestIds];

  if (createdAccountIds.length > 0) {
    const requestsByAccount =
      await prisma.foreignCurrencyRequest.findMany(
        {
          where: {
            accountId: {
    ...
    for (const requestRecord of requestsByAccount) {
      if (
        !requestIdsToDelete.includes(
          requestRecord.id,
        )
      ) {
        requestIdsToDelete.push(
          requestRecord.id,
        );
      }
    }
  ...
  /*
   * ------------------------------------------------------------------
   * 2. Delete ApprovalRecord rows first.
   *
   * ApprovalRecord references ForeignCurrencyRequest.
   * ------------------------------------------------------------------
   */
  if (
    requestIdsToDelete.length > 0
  ) {
    await prisma.approvalRecord.deleteMany(
      {
        where: {
          requestId: {
            in: requestIdsToDelete,
          },
        },
      },
    );
    ...
    /*
     * ----------------------------------------------------------------
     * 3. Delete audit logs associated with these FX requests.
     * ----------------------------------------------------------------
     */
    ...
    /*
     * ----------------------------------------------------------------
     * 4. Delete ForeignCurrencyRequest rows.
     *
     * This MUST happen before deleting the Account because:
     *
     * ForeignCurrencyRequest.accountId
     *             ↓
     *          Account
     * ----------------------------------------------------------------
     */
  ...
  /*
   * ------------------------------------------------------------------
   * 5. Delete any remaining audit logs created by these test users.
   * ------------------------------------------------------------------
   */
  ...
  /*
   * ------------------------------------------------------------------
   * 6. Delete account-dependent rows before Account.
   * ------------------------------------------------------------------
   */
  if (
    ...
    /*
     * Defensive second cleanup:
     *
     * If any FX request was still associated with one
     * of our accounts, remove it before attempting the
     * account deletion.
     */
    await prisma.foreignCurrencyRequest.deleteMany(
      {
  ...
  /*
   * ------------------------------------------------------------------
   * 7. Delete test currencies after FX requests are gone.
   * ------------------------------------------------------------------
   */
  if (
  ...
  /*
   * ------------------------------------------------------------------
   * 8. Delete user-dependent rows before User.
   * ------------------------------------------------------------------
   */
  if (createdUserIds.length > 0) {
...
describe(
  "ADMIN Approval Decision Authorization",
  () => {
    it("should allow ADMIN to approve a pending FX request", async () => {
      ...
      expect(
        response.body.data.request.reviewedBy,
      ).toBe(admin.id);

      expect(
        response.body.data.request.reviewNotes,
      ).toBe(
        "Documents verified successfully.",
      );
    });
      ...
      const response =
        await request(app)
          ...
          .send({
            ...
            comment:
              "Please provide the payment document.",
          });
